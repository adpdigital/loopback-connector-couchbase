...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  var options = {
    ...
    defaultExpiry: s.defaultExpiry || 0,
    env: s.env || 'debugging',
    connectionTimeout: s.connectionTimeout || 20,
    operationTimeout: s.operationTimeout || 15
  };

  //do not attach any port number such as 8901, will cause error code 24 which means "Error while establishing TCP connection"
  options.connectUrl = "couchbase://" + shuffle(options.host)
    + '?config_total_timeout=15&operation_timeout=10&config_node_timeout=5&durability_timeout=10&randomize_nodes=1&detailed_errcodes=1'
...
CouchbaseDB.prototype.connect = function (callback) {
  ...
  if (self.db) {
    ...
    debug("Initializing Couchbase Cluster via ", self.settings);
    ...
    self.myCluster = new couchbase.Cluster(self.settings.connectUrl, {
      ...
      // operation_timeout: self.settings.operationTimeout,
      // connectionTimeout: 10000,
      // connection_timeout: 500,
      // // durabilty_timeout: 10,
      // // views_timeout: 15,
      // detailed_errcodes: 1

    });
    // if (self.settings.username) {
    //   self.myCluster.authenticate(self.settings.username, self.settings.password)
    // }
...
var updateOneDoc = function(thisRef,myBucket,idkey,model,inboundData,callback){
  ...
  myBucket.get(idkey,function(err, result) {
    ...
    var meta = inboundData._meta || {}
    delete inboundData._meta
    meta.cas = result.cas
    meta.expiry = meta.expiry || self.settings.defaultExpiry
    var preparedData = updateDataBuilderObj.buildData();

    myBucket.upsert(idkey, preparedData, meta, function(err, result) {
      //myBucket.disconnect();
      if(err){
        self.checkReconnect(err);
        callback(err,null);
...
CouchbaseDB.prototype.query = function (where, callback) {
  ...
  self.getBucket(function(err, coll, myBucket) {
    if (err) {
      callback(err, null);
      return;
    }
    ...
    if (where.reduce !== undefined) {
      if (where.reduce === false) {
        options.reduce = 'false'
      } else {
        options.reduce = where.reduce
      }
    ...
    if (where.group_level) {
      options.group_level = where.group_level
    ...
    options.include_docs = where.include_docs
    options.timeout = 10000
    debug("CouchbaseDB.prototype.query() couchbase view query: " + JSON.stringify([options]))

    myBucket.viewQuery(designDocument, viewName, options, function (err, res) {
      if (err) {
        //myBucket.disconnect();
        self.checkReconnect(err);
...
CouchbaseDB.prototype.update = function (model, where, data, callback) {
  ...
  var myBucket = self.getBucket(function(err){
    ...
    myBucket.get(idkey,function(err, result) {
      ...
      var preparedData = data.updateFunc(result.value)
      var meta = preparedData._meta || {}
      delete preparedData._meta
      meta.expiry = meta.expiry || self.settings.defaultExpiry
      meta.cas = result.cas
...
CouchbaseDB.prototype.getBucket = function ( clbk ) {
  var self = this;
  if( !self.myBucket ) {
    self.bucket = self.myCluster.bucket(this.settings.bucket)
    self.myBucket = self.bucket.defaultCollection()
  ...
  process.nextTick(function () {
    clbk && clbk(null, self.myBucket, self.bucket)
  });
